...
export default function EmpCompany() {
  ...
  function validate() {
    const e = {}
    if (!form.name || form.name.trim().length < 2) e.name = 'Enter your company name'
    if (!form.location || form.location.trim().length < 2) e.location = 'Enter your company location'
    if (form.website && !form.website.includes('.')) e.website = 'Enter a valid website (e.g. acme.co.uk)'
    setErrors(e)
  ...
  async function continueOn() {
    ...
    // Delete any existing company for this user first, then insert fresh
    ...
    if (error || !company) {
      setErrors({ name: 'Could not save company. Please try again.' })
      setLoading(false)
      return
    }

    // Also save company_id to profile so PostJob can find it
  ...
  return (
    <>
      ...
      <div className="ob-wrap">
        ...
        <div className="input-row" style={{ borderColor: errors.name ? 'var(--red)' : '' }}>
        ...
        <div className="input-row" style={{ borderColor: errors.website ? 'var(--red)' : '' }}>
          <i className="ti ti-world"/>
          <input placeholder="Company website (e.g. acme.co.uk)" value={form.website} onChange={e => setForm({...form, website:e.target.value})}/>
        </div>
        {errors.website && <p style={{ fontSize:11, color:'var(--red)', marginTop:-6, marginBottom:8 }}>{errors.website}</p>}

        <div className="input-row" style={{ borderColor: errors.location ? 'var(--red)' : '' }}>
          <i className="ti ti-map-pin"/>
          <input placeholder="Company location * (e.g. London, UK)" value={form.location} onChange={e => setForm({...form, location:e.target.value})}/>
        </div>
        ...
        <div className="input-row">
          ...
          <select value={form.industry} onChange={e => setForm({...form, industry:e.target.value})}>
            {['Engineering & Infrastructure','Technology','Finance','Healthcare','Construction','Legal','Education','Retail','Other'].map(i => <option key={i}>{i}</option>)}
          </select>
        </div>
